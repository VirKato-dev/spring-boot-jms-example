...
public class ActiveMqProducer {
    ...
    public void convertAndSend() {
        switch (random.nextInt(3)) {
            case TEXT:
                jmsTemplate.convertAndSend(RabbitMqConfig.EXCHANGE_NAME, createTextMessage());
                break;
            case SERIALIZABLE:
                jmsTemplate.convertAndSend(RabbitMqConfig.EXCHANGE_NAME, createSerializableMessage());
                break;
            case MESSAGE_CREATOR:
                jmsTemplate.send(RabbitMqConfig.EXCHANGE_NAME, createMessageCreatorMessage());
                break;
            default:
